...
/******************************************************************************/
public class CMB_TYPEtrigger2 extends AttributeComputer {
	...
	public String compute(TreeWalker walker, ComputeContext context) {
		...
		String trigger_ce="TO_UNIXTIME(EVAL(PUT((BITAND([^:^:" + point1 + ".value],3)+BITAND({^:^:" + point2 + ".value},3)*2+IF({^:^:" + point2 + ".value}>4,4,0)),{^:dco.computedValue}),TO_UNIXTIME(PUT(IF({^:^:" + point2 + ".value}<4,{^:^:" + point2 + ":dac.acqDate},{^:^:" + point2 + ":dac.equipmentDate}),{^:dco.computedDate}))))";
		return trigger_ce;
	}
}
