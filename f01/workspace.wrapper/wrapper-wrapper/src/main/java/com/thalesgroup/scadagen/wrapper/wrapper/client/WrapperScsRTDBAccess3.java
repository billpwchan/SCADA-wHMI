...
public class WrapperScsRTDBAccess3 {
	
	private static Logger logger = Logger.getLogger(WrapperScsRTDBAccess3.class.getName());
	
	private static WrapperScsRTDBAccess3 instance = null;
	public static WrapperScsRTDBAccess3 getInstance() {
		if ( null == instance ) instance = new WrapperScsRTDBAccess3();
		return instance;
	}
	
	private ScsRTDBComponentAccess rtdb = null;
	private WrapperScsRTDBAccess3 () {
     ...
     * @param readResult
     */
    public void multiReadValueRequestCaches(String key, String scsEnvId, String[] dbaddresses, String resultName, ReadResult readResult) {
		logger.log(Level.SEVERE, "multiReadValueRequestCaches Begin");
		multiReadValueRequest(key, scsEnvId, dbaddresses, resultName, readResult);
    	...
    	while ( readRequests.hasNext() ) {
    		...
    		if ( readRequest.equalToKey(clientKey) ) {
    			...
    			for ( int i = 0 ; i < values.length ; ++i ) {
    				results[i] = new String[]{values[i], readRequest.dbaddresses[i]};
    			}
...
//    	multiReadValueRequest(readRequest);
    ...
    public void subscriptionDeRequest(String key) {
    	
    	logger.log(Level.SEVERE, "subscriptionDeRequest End");
    	
    	subscriptions.remove(new ReadRequest(key));
