...
chrome.runtime.onMessage.addListener(function (request, sender, callback) {
	...
	console.log("content event " + request.msg)
	...
	switch (request.msg) {
		...
		case "screenImg":
			...
			jscode = "thalesex_scrcap_on_captured ('";
			jscode += request.dataURI;
			jscode += "');";
...
var poller = window.setInterval(function() {
   ...
   if (document.documentElement.getAttribute('thalesex_scrcap_capture')) {
		document.documentElement.removeAttribute('thalesex_scrcap_capture');
		chrome.extension.sendMessage({"msg": "capturePage"}, function(res) {
		});
   }
   
}, 200);

// optional ui script injection
/*
var script = document.createElement('script');
script.src = chrome.extension.getURL('res/thalesex_scrcap_ui.js');
(document.head||document.documentElement).appendChild(script);
*/
