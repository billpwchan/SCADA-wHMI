...
$(TARGET): $(OBJECTS) $(APPLIBFILES)
	...
	$(LINK.obj) $(OBJECTS) $(LOADLIBES) $(OPT_MODE)PDB:$(basename $(TARGET))exe.pdb $(LDLIBS) /out:$@
	...
	@mv -f $@ $(BINDIR)/bin/
	@touch $@
	@if [ -e $(basename $(TARGET))exe.pdb ]; then \
	 mv -f $(basename $(TARGET))exe.pdb $(BINDIR)/bin/; \
	fi
...
clean:
	$(RM) $(REMOVEOBJS) $(TARGET) $(subst .exe,.bsc,$(TARGET)) $(subst .exe,.ilk,$(TARGET)) $(subst .obj,.sbr,$(OBJECTS)) $(subst .exe,.lib,$(TARGET)) $(subst .exe,.exp,$(TARGET)) *.idb *.pdb

clobber:
	$(RM) $(REMOVEOBJS) $(TARGET) $(subst .exe,.bsc,$(TARGET)) $(subst .exe,.ilk,$(TARGET)) $(subst .obj,.sbr,$(OBJECTS)) $(subst .exe,.lib,$(TARGET)) $(subst .exe,.exp,$(TARGET)) *.idb *.pdb $(TARGET).mak
