...
# Init
echo "Initializing SCADAsoft identification for QUICK-FIX..."
ident_rep=$ROOTDIR/src/ident/version

# Récupération du numéro de version
version=`sh $CONFDIR/bin/findversion.ksh`

# Récupération du niveau de patch courant et du commentaire associé
cd $ident_rep
...
QuickFix_comment="*** QUICK-FIX `date` generated from $view ***"
...
static char const SCS_QuickFix[]   = "\$ScsQuickFix: $QuickFix_comment $";
...
# Compilation
cd $ident_rep
##mkdir -p $BINDIR/src/ident/version
if [ "$SYSTEMFAMILY" = "win32" ]
then
    MAKE=make
else
...
then
    ...
    then
	echo "Initialization OK : $version (no patch) $QuickFix_comment"
    else
	echo "Initialization OK : $patch_level [$patch_comment] $QuickFix_comment"
    fi
else
