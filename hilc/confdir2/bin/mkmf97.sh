...
then
    ...
    makefile_template="$makefile_template.prg"
fi
...
do
	...
	;;
     ...
     if [ $library -ne 0 ]
     then
	 object="$target("`basename $source $extension`".o)"
     else
     ...
     $preprocessor $cppflags $includes $excludes $source > $tmp_cpp_output
     retcode=$?
     if [ $retcode -ne 0 ]
     then
...
then
    sed -e "s|%TARGET%|$target|g;\
      ...
      s|%DATE%|`date`|g;\
      s|%ID%|`whoami`|g" $tmp_makefile > $output
fi
