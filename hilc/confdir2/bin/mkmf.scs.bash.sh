...
# ---------------------------------------------------------------------------
#
command=`basename $0 .scs.bash.sh`
...
then
    ...
    preprocessor="cc -E"
    includes=""
    excludes="-I/usr/include -I/usr/lib"
fi
cppflags=
filetrace=
datadir=`dirname $0`
...
do
    ...
    if [ -n "$ac_prev" ]
    then
        if [[ "$ac_prev" == "includes" || "$ac_prev" == "excludes" ]]
        then
            eval "$ac_prev=\"\$$ac_prev $ac_option\""
        else
...
trap "rm -f $tmp_makefile" 1 2 3 6 15

cat $makefile_template > $tmp_makefile

object_list=
retcode=0
paramsFile=${tmpdir}/params$$.txt
echo "" > $paramsFile

# Function used for checking a simple dependency
# param1[in]    source file
# param2[in]    object file
# param3[in]    dependency regexp
# param4[in]    output temporary dependency file
# return 0 on success, 255 on error (because XARGS stop when return code of command is 255)
depends_fn() {
    pSource=$1
    pObject=$2
    pDependRegex=$3
    pOutput=$4
    outputTmp=${pOutput}.tmp

    trap "rm -f $pOutput $outputTmp" 0
    
    echo "==== Parsing $pSource ====" | tee -a $filetrace > /dev/null
    # Use SH here because bash seems to crash
    sh -c "$preprocessor $cppflags $includes $excludes $pSource > $outputTmp"
    retcode=$?
    if [ $retcode -ne 0 ]; then

        cat 1>&2 << EOF_CPP_ERROR
$command: error: $pSource cannot be preprocessed.
EOF_CPP_ERROR
        echo "==== Stopped in $pSource ====" | tee -a $filetrace > /dev/null
        retcode=255

    else

        #  Parse result
        cat $outputTmp | tee -a $filetrace | \
            egrep "$egrep_regexp" 2>/dev/null | \
            $awk_command \
            EXCLUDED_DIRS="$excludes" \
            DEPENDENCY_REGEXP="$pDependRegex" \
            OBJECT="$pObject" > $pOutput
        echo >> $pOutput
        retcode=0
    fi
...
do
    ...
    *)	continue
    ...
    # Preparing parameters only
    dependency_maktmp=${tmpdir}/`basename $source $extension`.$$maktmp
    echo "$source $object \'$dependency_regexp\' $dependency_maktmp" >> $paramsFile
done


# Useful variables
export command awk_command
export preprocessor cppflags includes excludes egrep_regexp filetrace
export -f depends_fn
trap "rm -f ${tmpdir}/*.$$maktmp ${paramsFile}" 0

# Number of parallel tasking
nb_proc=1
if [ -n "$PARALLELMAKEOPTION" ]; then
    nb_proc=`echo $PARALLELMAKEOPTION | cut -c3-`
fi
# Execute parallel tasking (and avoid "xargs: argument line too long")
cat $paramsFile | xargs -P$nb_proc -n4 -i{} bash -c depends_fn\ \{\}
retcode=$?

# Variables cleaning
export -n command awk_command
export -n preprocessor cppflags includes excludes egrep_regexp filetrace
export -n depends_fn


if [ $retcode -eq 0 ]
then
    cat ${tmpdir}/*.$$maktmp >> $tmp_makefile 2> /dev/null
