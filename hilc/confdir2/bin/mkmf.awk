...
EXCLUDED_DIRS != "" {
    ...
    sub(/^(-|\/)I/, "", excluded_dirs[i]);          # Remove -I or /I
    gsub(/(\\)+/, "/", excluded_dirs[i]);           # Change all one or multiple \ to /
    excluded_dirs[i] = tolower(excluded_dirs[i]);   # Lower case for easier test
#    printf(" #%s ", excluded_dirs[i]);
    }
    EXCLUDED_DIRS = "";
#    printf("\n");
}
...
($1 ~ /#.*/ && $2 ~ /[0-9][0-9]*/ && $3 ~ dependency_regexp) {
    ...
    dependency_totest = tolower(dependency);
    gsub(/(\\)+/, "/", dependency_totest);

    is_excluded = 0;
    # Test for dependency exclusion
    for(i = 1; i <= excluded_dirs_count; i++) {
	if(index(dependency_totest, excluded_dirs[i]) == 1) {
	    is_excluded = 1;
	    break;
	}
    }
    # Test for unique dependency result
